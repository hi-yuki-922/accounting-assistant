...
**决策**：Chatbot 页面使用 shadcn/ui Resizable 组件划分为左右两部分，默认宽度比例 6:4，右侧最小宽度 320px。左侧为空容器占位，由后续任务看板提案填充。

**理由**：
- 左侧任务看板是确定要做的功能，提前划分好布局区域避免后续大改
- Resizable 允许用户根据使用场景自由调节宽度，适应不同屏幕尺寸
- 右侧 320px 最小宽度保证对话区域的基本可用性
- 左右两部分内部结构独立，互不影响
