...
- [x] 8.2 验证所有 14 个工具可被 Agent 正确调用（通过 ToolLoopAgent 执行工具调用测试）
- [x] 8.3 验证 Section 存储流程：创建会话 → 创建节 → 写入消息 → 读取消息 → 生成摘要
- [x] 8.4 验证路由函数：无引用新建节、有引用续接节、历史摘要注入
- [x] 8.5 验证提示词动态加载：修改 Markdown 文件后重新创建 Agent，提示词内容更新
