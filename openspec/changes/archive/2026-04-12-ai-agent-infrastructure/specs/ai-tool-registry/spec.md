...
系统 SHALL 将 Agent 工具按 4 个业务域分类定义，每个工具定义在对应的 TypeScript 文件中：
...
- **THEN** 包含 `index.ts`、`basic-data.ts`、`order.ts`、`accounting.ts`、`system.ts` 五个文件
...
### Requirement: 工具统一导出
`src/ai/tools/index.ts` SHALL 导出一个 `getAllTools()` 函数，返回所有分类工具的合集对象，供 Agent 使用。

#### Scenario: getAllTools 返回全部工具
- **WHEN** 调用 `getAllTools()`
- **THEN** 返回包含 14 个工具的对象，key 为工具名，value 为工具定义

### Requirement: 工具动态加载支持
`src/ai/tools/index.ts` SHALL 支持按类别加载工具子集，提供 `getToolsByCategory(category)` 函数。
...
- `create_order` — 创建订单，调用 `orderApi.createOrder(...)`，入参包含订单类型、客户、商品明细列表
...
- **THEN** 入参包含必填的 `orderType`（Sales/Purchase）、`items`（商品明细数组），可选的 `customerId`、`remark`、`actualAmount`、`subType`
