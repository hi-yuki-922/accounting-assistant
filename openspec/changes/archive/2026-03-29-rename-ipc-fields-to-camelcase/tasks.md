...
- [x] 7.1 启动完整应用 `pnpm tauri dev`
- [x] 7.2 测试账本管理功能：创建、查询、更新、删除账本
- [x] 7.3 测试记账记录功能：添加、修改、过账、查询记录
- [x] 7.4 测试附件管理功能：上传、查询、下载、删除附件
- [x] 7.5 测试聊天功能：创建会话、发送消息、查询记录
- [x] 7.6 检查浏览器控制台，确保无错误日志
- [x] 7.7 验证所有数据传输前后端字段名匹配

## 8. 代码审查和清理

- [x] 8.1 运行 `pnpm dlx ultracite check` 检查前端代码质量
- [x] 8.2 运行 `cargo clippy` 检查后端代码质量
- [x] 8.3 审查所有修改的代码，确保无遗漏或误改
- [x] 8.4 确认全大写常量（如 `DEFAULT_BOOK_ID`、`PAGINATION`）未被修改
