...
use dto::{CreateSessionDto, CreateMessageDto};
...
impl ChatService {
    ...
    /// 创建新会话
    pub async fn create_session(&self, input: CreateSessionDto) -> Result<SessionModel, Box<dyn std::error::Error>> {
    ...
    /// 根据 ID 获取会话
    pub async fn get_session_by_id(&self, id: i64) -> Result<Option<SessionModel>, Box<dyn std::error::Error>> {
        let session = chat_session::Entity::find_by_id(id).one(&self.db).await?;
        Ok(session)
    }

    /// 更新会话标题
    pub async fn update_session_title(&self, id: i64, title: String) -> Result<SessionModel, Box<dyn std::error::Error>> {
        let session = chat_session::Entity::find_by_id(id)
            .one(&self.db)
            .await?
            .ok_or::<String>("Session not found".into())?;
    ...
    /// 删除会话（级联删除该会话的所有消息）
    pub async fn delete_session(&self, id: i64) -> Result<DeleteResult, Box<dyn std::error::Error>> {
        // 先删除该会话的所有消息
        ...
        // 再删除会话
        let result = chat_session::Entity::delete_by_id(id).exec(&self.db).await?;
        Ok(result)
    }

    /// 添加消息
    pub async fn add_message(&self, input: CreateMessageDto) -> Result<MessageModel, Box<dyn std::error::Error>> {
    ...
    pub async fn update_message_state(
        ...
        let message = chat_message::Entity::find_by_id(id)
            .one(&self.db)
            .await?
            .ok_or::<String>("Message not found".into())?;
