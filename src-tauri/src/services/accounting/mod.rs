...
use crate::enums::{AccountingRecordState, AccountingType, AccountingChannel};
use chrono::Local;
...
use self::dto::{AddAccountingRecordDto, ModifyAccountingRecordDto, CreateWriteOffRecordDto};
...
impl AccountingService {
    ...
    pub async fn delete_record(&self, id: i64) -> Result<(), Box<dyn std::error::Error>> {
        ...
        // 删除记录
        accounting_record::Entity::delete_by_id(id).exec(&self.db).await?;
    ...
    /// 批量入账（预验证 + 事务保证原子性）
    pub async fn batch_post_records(&self, record_ids: Vec<i64>) -> Result<Vec<Model>, Box<dyn std::error::Error>> {
        ...
        if records.len() != record_ids.len() {
            ...
            let missing_ids: Vec<i64> = record_ids.iter()
    ...
    pub async fn create_write_off_record(
        ...
        // 转换金额
        let amount = Decimal::from_f64_retain(input.amount)
            .ok_or_else(|| "无效的冲账金额".to_string())?;
        ...
        let total_write_off: Decimal = existing_write_offs.iter()
            .map(|r| r.amount)
            .sum();
        ...
        let channel = if let Some(channel_str) = input.channel {
            channel_str.parse::<AccountingChannel>()
