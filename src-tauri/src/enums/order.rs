...
impl TryGetable for OrderType {
    fn try_get_by<I: sea_orm::ColIdx>(res: &sea_orm::QueryResult, idx: I) -> Result<Self, sea_orm::TryGetError> {
        ...
        value.parse::<OrderType>()
            .map_err(|_| sea_orm::TryGetError::DbErr(DbErr::Type(String::from("Invalid OrderType"))))
    }
...
impl TryGetable for OrderStatus {
    fn try_get_by<I: sea_orm::ColIdx>(res: &sea_orm::QueryResult, idx: I) -> Result<Self, sea_orm::TryGetError> {
        let value: String = res.try_get_by(idx).map_err(sea_orm::TryGetError::DbErr)?;
        value.parse::<OrderStatus>()
            .map_err(|_| sea_orm::TryGetError::DbErr(DbErr::Type(String::from("Invalid OrderStatus"))))
    }
...
impl sea_orm::TryFromU64 for OrderStatus {
    fn try_from_u64(_n: u64) -> Result<Self, DbErr> {
        Err(DbErr::Type(String::from("Cannot convert u64 to OrderStatus")))
    }
}
