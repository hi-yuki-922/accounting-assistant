...
export const createZAiProvider = tryResult(() => {
  ...
  return createZhipu({ apiKey, baseURL: 'https://open.bigmodel.cn/api/paas/v4' })
})

const prompt = `你是一个专业的财务助手，可以帮助用户进行记账、财务分析和财务咨询。
## 你的能力
1. 查询和分析用户的财务数据
2. 添加新的记账记录
3. 提供个性化的财务建议
...
export const createFinanceAgent = (
  ...
  createZAiProvider().map(zAi => new ToolLoopAgent({
    instructions: prompt,
    model: zAi(modelName),
    toolChoice: 'auto',
    tools: financeTools,
