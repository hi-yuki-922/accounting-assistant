...
pub async fn settle_order(&self, input: SettleOrderDto) -> Result<(), Box<dyn std::error::Error>> {
    // 开启事务
    let txn = self.db.begin().await?;

    // 执行多个操作
    let order = Entity::find_by_id(input.order_id).one(&txn).await?
        .ok_or("订单不存在")?;

    // 更新操作...
    let mut active: ActiveModel = order.into();
    active.status = ActiveValue::Set(OrderStatus::Settled);
...
每个主实体都有对应的 `_seq` 表，用于原子 ID 生成。不同实体的序列表结构有差异：

### 标准序列表（复合主键）

大多数业务实体（如 `customer_seq`、`product_seq`、`order_seq`）使用 `date_key` + `seq` 复合主键：
...
### chat_message_seq（单 i64 主键）

聊天消息序列表使用 `id: i64` 单主键，额外包含 `date_key` 和 `seq` 字段：
...
> **注意**：`section_summary` 实体复用 `chat_message_seq` 进行 ID 生成，而非拥有独立的序列表。

### category_seq（单 i32 主键）

品类序列表使用 `id: i32` 作为日期键：
